    ...
    if longPoll['updates'] and len(longPoll['updates']) != 0:
        for update in longPoll['updates']:
            if update['type'] == 'message_new':
                ...
                api.messages.send(user_id = update['object']['user_id'], random_id = randint(-2147483648, 2147483647), message = 'Привет, %s &#128521;'%name, attachment = 'doc%s_%s,photo%s_%s'%(doc['owner_id'], doc['id'], doc['owner_id'], photo['id']))
